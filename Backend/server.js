...
app.post("/api/signup", (req, res) => {
  ...
  db.query(sql, [fullname, email, username, password], (err, results) => {
    ...
    db.query(sqlCreateUserCartTable, (err) => {
      ...
      res.json({
        success: true,
        message: "Sign up successful and user-specific table created!",
      });
    });
  });
});
...
app.post("/api/login", (req, res) => {
  ...
  // Query to retrieve user details from users table
  ...
  db.query(sql, [username], (err, results) => {
    ...
    if (password === user.password) {
      // Insert or update user details in user_details table
      const insertSql = `
        INSERT INTO user_details (id, username, password, user_id)
        VALUES (?, ?, ?, ?)
      ...
      db.query(
        insertSql,
        [user.id, username, password, user.id],
        (insertErr) => {
          if (insertErr) {
            console.error("Error inserting/updating user details:", insertErr);
...
app.post("/api/out", (req, res) => {
  ...
  // First, get all active usernames from user_details to clear their data
  const getUsersQuery = "SELECT username FROM user_details";
  db.query(getUsersQuery, (err, users) => {
    if (err) {
      console.error("Error fetching users from user_details:", err);
      // Continue with logout even if query fails
    }

    // Delete from user_details table
    const deleteAllSql = "DELETE FROM user_details";
    db.query(deleteAllSql, (err, result) => {
      if (err) {
        console.error("Error deleting user details:", err);
        return res.status(500).json({
          success: false,
          message: "Unable to clear user details. Please try again later.",
        });
      }

      console.log("Delete result:", result); // Logs affectedRows

      // Clear the user's cart table - delete from all active users' carts
      let cartsClearedCount = 0;
      if (users && users.length > 0) {
        users.forEach((user) => {
          const cartTableName = `user_cart_${user.username.replace(
            /[^a-zA-Z0-9_]/g,
            "_"
          )}`;
          const deleteCartSql = `DELETE FROM ${cartTableName}`;

          db.query(deleteCartSql, (err) => {
            if (err) {
              console.error(`Error clearing cart table ${cartTableName}:`, err);
            }
            cartsClearedCount++;

            // Check if all carts have been processed
            if (cartsClearedCount === users.length) {
        ...
        // No users found, just return success
        if (result.affectedRows === 0) {
          return res
            .status(200)
            .json({ success: true, message: "No records to delete." });
        }
  ...
  connection.query("SELECT name FROM restaurant", (err, restaurants) => {
    ...
    const promises = restaurants.map((restaurant) => {
      return new Promise((resolve, reject) => {
        ...
        connection.query(
          `SELECT * FROM \`${restaurant.name}\` WHERE item_name = ?`,
          [item],
          (err, menu) => {
...
app.post("/api/addRestaurant", upload.single("shopImage"), (req, res) => {
  ...
  const sqlInsert =
    "INSERT INTO restaurant (name, address, open, close, contact, image) VALUES (?, ?, ?, ?, ?, ?)";
  db.query(
    sqlInsert,
    [shopName, shopAddress, shopOpens, shopClose, shopContact, shopImage],
    (err, results) => {
      if (err) {
        console.error("Error inserting data into database:", err);
        return res
      ...
      db.query(sqlCreateTable, (err) => {
        ...
        res.json({
          success: true,
          message: "Shop added and table created successfully!",
        });
      });
    }
  );
...
app.get("/api/getRestaurants", (req, res) => {
  const sql =
    "SELECT id, name, address, open, close, contact, image FROM restaurant";
  ...
  db.query(sql, (err, results) => {
    ...
    const modifiedResults = results.map((restaurant) => {
      if (restaurant.image) {
        restaurant.image = `data:image/jpeg;base64,${Buffer.from(
          restaurant.image
        ).toString("base64")}`;
      }
      return restaurant;
  ...
  db.query(sqlGetRestaurantId, [shopName], (err, results) => {
    ...
    db.query(sqlCheckTable, [sanitizedTableName], (err, results) => {
      ...
      if (results.length === 0) {
        return res
          .status(400)
          .json({ success: false, message: "Table does not exist" });
      }
...
app.get("/api/getMenu/:shopName", (req, res) => {
  ...
  db.query(sql, (err, results) => {
    ...
    const modifiedResults = results.map((item) => {
      if (item.food_image) {
        item.food_image = `data:image/jpeg;base64,${Buffer.from(
          item.food_image
        ).toString("base64")}`;
      }
      return item;
  ...
  db.query(userQuery, (err, results) => {
    ...
    db.beginTransaction((err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ success: false, error: "Transaction error" });
      }
        ...
        (err) => {
          if (err) {
            ...
            return db.rollback(() => {
              res.status(500).json({
                success: false,
                error: "Error inserting data into user cart",
              });
            });
          }
          res.status(200).json({
...
app.get("/api/getCartItems/:username", (req, res) => {
  ...
  db.query(cartQuery, (err, cartResults) => {
    ...
    cartResults.forEach((item) => {
      const restaurantTableName = item.restaurant_name.replace(
        /[^a-zA-Z0-9_]/g,
        "_"
      );
      ...
      db.query(imageQuery, [item.menu_item], (err, imageResults) => {
        ...
        const foodImage = imageResults.length
          ? `data:image/jpeg;base64,${Buffer.from(
              imageResults[0].food_image
            ).toString("base64")}`
          : "";
...
app.delete("/api/removeCartItem/:username/:menu_item", (req, res) => {
  ...
  if (!menuItem) {
    ...
    return res
      .status(400)
      .json({ success: false, message: "Menu item name is required" });
  }

  console.log(
    `Attempting to delete from ${cartTableName} where menu_item = ${menuItem}`
  );
...
app.post("/api/placeOrder", (req, res) => {
  ...
  if (typeof grandTotal !== "number" || isNaN(grandTotal) || grandTotal <= 0) {
    return res
      .status(400)
      .json({ success: false, message: "Invalid grand total" });
  }
  if (typeof tokensToAdd !== "number" || tokensToAdd <= 0) {
    return res
      .status(400)
      .json({ success: false, message: "Invalid tokens value" });
  }
  ...
  db.beginTransaction((err) => {
    ...
    const insertOrderQuery =
      "INSERT INTO orders (username, grand_total, tokens_to_add, order_time) VALUES (?, ?, ?, NOW())";
    db.query(
      insertOrderQuery,
      [username, grandTotal, tokensToAdd],
      (error, results) => {
        if (error) {
          ...
          return db.rollback(() => {
            res
              .status(500)
              .json({ success: false, message: "Failed to insert order" });
          });
        }
        ...
        const insertOrderItemsQuery =
          "INSERT INTO order_items (order_id, item_id, item_name, quantity, price, total, username) VALUES ?";
          ...
          db.query(updateTokensQuery, [tokensToAdd, username], (error) => {
            if (error) {
              ...
              return db.rollback(() => {
                res
                  .status(500)
                  .json({ success: false, message: "Failed to update tokens" });
              });
            }
  ...
  db.query(tokenQuery, [userId], (err, results) => {
    ...
    if (!results || results.length === 0) {
      return res.status(404).json({
        success: false,
        message: "Tokens not found for the provided user ID",
      });
    }
...
app.post("/api/donateTokens", (req, res) => {
  ...
  if (!grandTotal || isNaN(grandTotal) || grandTotal <= 0) {
    ...
    return res
      .status(400)
      .json({ success: false, message: "Invalid grand total amount" });
  }
  ...
  db.query(tokenQuery, [userId], (err, results) => {
    ...
    if (currentTokens < grandTotal) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Insufficient tokens to donate" });
    }
...
const transporter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: "cvinayak87@gmail.com",
    pass: "eazc nosh xawb dhna", // Gmail app password
  },
});
...
app.post("/api/sendConfirmationEmail", (req, res) => {
  ...
  const mailOptions = {
    from: "cvinayak87@gmail.com",
    to: email,
    subject: "🎉 Welcome to NutriSafe Newsletter - Exclusive Offers Inside!",
    html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px;">
        ...
        <!-- Header -->
        <div style="text-align: center; padding: 30px 20px; background: white; border-radius: 15px 15px 0 0;">
          <h1 style="color: #667eea; font-size: 32px; margin: 0; font-weight: bold;">🥗 NutriSafe</h1>
          <p style="color: #764ba2; font-size: 16px; margin: 10px 0 0 0;">Your Allergen-Friendly Companion</p>
        </div>

        <!-- Main Content -->
        <div style="background: white; padding: 40px 30px; text-align: center;">
          <h2 style="color: #667eea; font-size: 24px; margin: 0 0 20px 0;">🎊 Welcome Aboard, Subscriber! 🎊</h2>
          
          <p style="color: #333; font-size: 16px; line-height: 1.8; margin: 20px 0;">
            Thank you for subscribing to our newsletter! You're now part of the NutriSafe community where we celebrate <strong>healthy eating</strong>, <strong>allergen awareness</strong>, and <strong>delicious discoveries</strong>! 🌟
          </p>

          <!-- Coupon 1 -->
          <div style="background: linear-gradient(135deg, #FFD700 0%, #FFA500 100%); border-radius: 12px; padding: 25px; margin: 25px 0; box-shadow: 0 4px 15px rgba(255, 165, 0, 0.3);">
            <p style="color: white; font-size: 14px; margin: 0 0 10px 0; text-transform: uppercase; font-weight: bold;">🎁 Exclusive Welcome Offer</p>
            <div style="background: white; border-radius: 8px; padding: 15px; margin: 10px 0;">
              <p style="color: #333; font-size: 28px; margin: 0; font-weight: bold; letter-spacing: 2px;">NEWBEE!</p>
            </div>
            <p style="color: white; font-size: 15px; margin: 10px 0 0 0; font-weight: bold;">
              Get up to <span style="font-size: 22px;">₹100</span> discount on your first order above ₹399! 🚀
            </p>
          </div>

          <!-- Coupon 2 -->
          <div style="background: linear-gradient(135deg, #00D084 0%, #00A86B 100%); border-radius: 12px; padding: 25px; margin: 25px 0; box-shadow: 0 4px 15px rgba(0, 168, 107, 0.3);">
            <p style="color: white; font-size: 14px; margin: 0 0 10px 0; text-transform: uppercase; font-weight: bold;">💚 Loyalty Reward</p>
            <div style="background: white; border-radius: 8px; padding: 15px; margin: 10px 0;">
              <p style="color: #333; font-size: 28px; margin: 0; font-weight: bold; letter-spacing: 2px;">SPECIAL20</p>
            </div>
            <p style="color: white; font-size: 15px; margin: 10px 0 0 0; font-weight: bold;">
              20% off (up to ₹100) on orders above ₹500! 💎<br>
              <span style="font-size: 12px; font-style: italic;">Valid once per user</span>
            </p>
          </div>

          <!-- Benefits Section -->
          <div style="background: #F8F9FA; border-radius: 12px; padding: 20px; margin: 25px 0; text-align: left;">
            <h3 style="color: #667eea; text-align: center; margin-top: 0;">What You Get as a Subscriber:</h3>
            <ul style="color: #333; font-size: 15px; line-height: 2; margin: 15px 0; padding-left: 20px;">
              <li>🛍️ Exclusive deals & early access to special offers</li>
              <li>🍽️ Curated restaurant & food recommendations</li>
              <li>🥗 Allergen-free meal suggestions tailored to you</li>
              <li>💰 Member-only discounts and promotions</li>
              <li>📰 Weekly updates on healthy eating tips</li>
              <li>👥 Community insights & success stories</li>
            </ul>
          </div>
          ...
          <div style="margin: 30px 0;">
            <p style="color: #555; font-size: 16px; margin: 0 0 15px 0;">
              Ready to explore? Visit NutriSafe now! 👇
            </p>
            <a href="http://localhost:5500/Frontend/index.html" style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 15px 40px; border-radius: 50px; text-decoration: none; font-weight: bold; font-size: 16px; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.4);">
              Start Shopping 🛒
            </a>
          </div>
        </div>
        ...
        <!-- Footer -->
        <div style="background: #333; color: white; padding: 20px 30px; text-align: center; border-radius: 0 0 15px 15px; font-size: 12px;">
          <p style="margin: 0 0 10px 0;">
            <strong>NutriSafe</strong> - Nourishing Confidence, Connecting Communities 🌍
          </p>
          <p style="color: #AAA; margin: 10px 0 0 0;">
            Questions? Contact us at <a href="mailto:Nutrisafe@gmail.com" style="color: #667eea; text-decoration: none;">Nutrisafe@gmail.com</a>
          </p>
          <p style="color: #AAA; margin: 5px 0 0 0;">
            © 2025 NutriSafe. All rights reserved.
          </p>
        </div>
...
app.post("/api/applyCoupon", async (req, res) => {
  try {
    ...
    if (typeof grandTotal !== "number") {
      return res
        .status(400)
        .json({ success: false, message: "Grand total must be a number." });
    }

    db.query(
      "SELECT username FROM user_details LIMIT 1",
      async (err, userResults) => {
        ...
        // Fetch user orders
        db.query(
          "SELECT COUNT(*) AS orderCount FROM orders WHERE username = ?",
          [username],
          (err, orderResults) => {
            if (err) {
            ...
            if (couponCode === "NEWBEE!") {
              ...
              if (grandTotal >= 500) {
                const discountAmount = Math.min(grandTotal * 0.2, 100); // Calculate 20% or ₹100, whichever is lowe
                const discountedTotal =
                  grandTotal - Math.min(grandTotal * 0.2, 100);
                const finalTotal = discountedTotal < 0 ? 0 : discountedTotal;
                res.json({
                  success: true,
                  message: "Coupon applied successfully.",
...
app.get("/api/getRestaurantsByProduct", async (req, res) => {
  ...
  if (!product) {
    return res
      .status(400)
      .json({ success: false, message: "Product parameter is missing" });
  }
  ...
  db.query(sql, (err, results) => {
    ...
    const formattedNames = restaurantNames.map((name) =>
      name.replace(/ /g, "_")
    );
